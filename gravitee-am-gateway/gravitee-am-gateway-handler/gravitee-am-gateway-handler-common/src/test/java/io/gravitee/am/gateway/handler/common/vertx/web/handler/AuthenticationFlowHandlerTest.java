...
public class AuthenticationFlowHandlerTest extends RxWebTestBase {
    ...
    @Test
    public void shouldNotRedirectToMFAChallengePage_adaptiveMFA_strongly_auth() throws Exception {
        router.route().order(-1).handler(rc -> {
            // set client
            Client client = new Client();
            client.setFactors(Collections.singleton("factor-1"));
    ...
    @Test
    public void shouldNotRedirectToMFAChallengePage_adaptiveMFA_2_strongly_auth() throws Exception {
        router.route().order(-1).handler(rc -> {
            // set client
            Client client = new Client();
            client.setFactors(Collections.singleton("factor-1"));
